...
class Highcharts{
	...
	function push_serie_data($s_serie_name = '', $s_value=''){
		if($s_serie_name !== '' && $s_value !=='' ){
			$f=false;
			foreach($this->a_options['series'] as $index => $serie){
				//echo strtolower($serie['name'])." == ".strtolower($s_serie_name)."<br>";
				if(strtolower($serie['name']) == strtolower($s_serie_name)){
					$f=$index;
					break;
				}
	...
	/* Render */
	function render($width=FALSE,$height=FALSE,$=FALSE){
		...
		$this->s_rendering .= '});';
		$this->s_rendering .= '</script>';
		$this->s_rendering .= '<div id="'.$this->a_options['chart']['renderTo'].'" style="width:'.$width.';height:'.$height.'"></div>';
